...
func play(new_zone: int, index: int):
	...
	if to_set.zone == Zone.HAND:
		var should_set_e_mark = false
		if (to_set.type == Type.BATTLE and to_set.attribute != Attribute.WEAPON) or \
			to_set.type == Type.SITUATION:
		...
		if to_set.uses_one_card_per_turn():
			if to_set.type == Type.BATTLE:
				player.used_one_battle_card_per_turn = true
			elif to_set.type == Type.SITUATION:
