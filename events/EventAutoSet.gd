...
func _init(_game_board: GameBoard, _player: Player, _to_set: Card, _zone: int, _index: int):
	super(_game_board)
	player = _player
	to_set = _to_set
	zone = _zone
...
func on_start():
	if to_set.zone in [Zone.STANDBY, Zone.BATTLEFIELD, Zone.SITUATION]:
		return
	...
	if to_set.type == Type.BATTLE and to_set.attribute == Attribute.WEAPON:
...
func play(new_zone: int, index: int):
	...
	if to_set.type != Type.BATTLE or to_set.attribute != Attribute.WEAPON:
...
func handle_occupied_zone(zone: int, index: int):
	for e in to_set.get_effects():
