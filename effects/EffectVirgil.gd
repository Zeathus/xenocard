...
func adjust_enemy():
	...
	for c in card.owner.field.get_battlefield_cards():
		if c.attribute == Card.Attribute.REALIAN:
			events.push_back(EventDestroy.new(card.owner.game_board, card, c, Card.DamageSource.EFFECT))
			events.push_back(EventDamage.new(card.owner.game_board, card, card.owner.get_enemy(), 4, Card.DamageSource.EFFECT))
